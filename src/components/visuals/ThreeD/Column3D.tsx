...
interface PropsBase { inputs: ColumnInputs; results: ColumnResults }
...
function ColumnMesh({ inputs, results, onFit, still }: MeshProps) {
  ...
  return (
    <group ref={groupRef}>
      {/* Frame the full column height (+ headroom for the labels) */}
      <FitCamera sx={W} sy={H + 1} sz={isCirc ? W : D} onFit={onFit} />
    ...
    <div className="w-full rounded-xl overflow-hidden bg-gradient-to-b from-slate-800 to-slate-900 shadow-lg ring-1 ring-slate-700/50" style={{ height: 380 }}>
      <Canvas shadows camera={{ fov: 45 }} gl={{ preserveDrawingBuffer: true }}>
        ...
        <Suspense fallback={null}>
          <ColumnMesh {...props} onFit={handleFit} />
