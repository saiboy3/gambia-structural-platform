...
interface PropsBase { inputs: FoundationInputs; results: FoundationResults }
...
function FoundationMesh({ inputs, results, onFit, still }: MeshProps) {
  const groupRef = useRef<THREE.Group>(null);
  useFrame((_, delta) => {
    if (!still && groupRef.current) groupRef.current.rotation.y += delta * 0.25;
  });
